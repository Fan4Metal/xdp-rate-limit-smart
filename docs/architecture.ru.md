...
3. `load_existing_bans()` — прочитать `blacklist_map`, отбросить уже истёкшие
   записи, а остальные запомнить, чтобы перезапуски не теряли и не дублировали баны.
...
`Restart=on-failure` возвращает демон после креша; поскольку и `load`, и загрузка
банов в демоне идемпотентны и самолечащи, перезапуск чисто переприсоединяется и
подхватывает всё ещё активные баны, а не громоздит дубликаты XDP-программ. Неуклонно
растущий `NRestarts` — сигнал разобраться с настоящим циклом перезапусков.
